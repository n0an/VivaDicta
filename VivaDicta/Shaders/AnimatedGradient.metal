...
/// A shader that generates a constantly cycling color gradient, centered on the input view.
///
...
[[ stitchable ]] half4 animatedGradientFill(float2 position, half4 color, float2 size, float time) {
    ...
    // Get the same UV in the range -1 to 1, so that 0 is in the center.
    half2 rp = uv * 2.0h - 1.0h;

    // Wrap the time value to prevent it from becoming too large
    float wrappedTime = fmod(time, 2.0 * 3.1415927);

    // Calculate the angle to this pixel, adding in time so it's constantly changing.
    half angle = atan2(rp.y, rp.x) + wrappedTime;

    // Send back variations on the sine of that angle for a range of colors.
