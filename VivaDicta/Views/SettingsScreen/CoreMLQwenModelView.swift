...
//
//  Cards (one per on-device Qwen variant) shown inline in the AI Providers
//  "Local" tab, below the Gemma cards. Parallel to LiteRTGemmaModelView - same
//  card style + circular download/cancel/delete button. (Shared LocalLLM
//  abstraction across the two backends is a later follow-up.)
//
...
/// One card per on-device Qwen variant, shown inline in the AI Providers
/// "Local" tab below the Gemma cards.
struct QwenVariantCard: View {
    ...
    private var status: CoreMLQwenModelViewModel.Status { model.status(for: variant) }
