...
struct MultiNoteChatView: View {
    ...
    @ViewBuilder
    private func citationPillRow(for sources: [MultiNoteSourceCitationDisplay]) -> some View {
        HStack(spacing: 6) {
    ...
    private func sourceLabel(for source: MultiNoteSourceCitationDisplay) -> String {
    ...
    private func resolveCitationDisplays(for message: ChatMessage) -> [MultiNoteSourceCitationDisplay] {
        ...
        if !citations.isEmpty {
            ...
            return citations
                ...
                .compactMap { citation in
                    ...
                    return MultiNoteSourceCitationDisplay(transcription: transcription, citation: citation)
                }
        }

        return resolveSourceTranscriptions(ids: message.sourceTranscriptionIds).map { transcription in
            MultiNoteSourceCitationDisplay(transcription: transcription, citation: nil)
        }
    }
}
