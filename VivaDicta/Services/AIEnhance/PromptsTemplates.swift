...
extension PromptsTemplates {
    static func systemPrompt(with instructions: String) -> String {
        ...
        1. Always reference <CLIPBOARD_CONTEXT>, <CURRENTLY_SELECTED_TEXT>, and <CUSTOM_VOCABULARY> for better accuracy if available, because the <TRANSCRIPT> text may have inaccuracies due to speech recognition errors.
        2. When similar phonetic occurrences are detected between words in the <TRANSCRIPT> text and terms in <CUSTOM_VOCABULARY>, <CLIPBOARD_CONTEXT>, or <CURRENTLY_SELECTED_TEXT> - prioritize the spelling from these context sources over the <TRANSCRIPT> text.
