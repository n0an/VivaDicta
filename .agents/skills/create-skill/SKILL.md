---
name: create-skill
description: Create a new skill in .agents/skills/ with proper structure and a compatibility symlink in .claude/skills/
...
### 2. Create the skill directory and main file
...
Then create a compatibility symlink:
...
- See `../related-skill-1/SKILL.md` if [specific condition]
- See `../related-skill-2/SKILL.md` for [specific relationship]
...
### 3. Keep the compatibility mirror in sync

This repository does not maintain a separate local skill index file.

- Ensure the real skill directory exists under `.agents/skills/<skill-name>/`
- Ensure `.claude/skills/<skill-name>` is a symlink pointing to `../../.agents/skills/<skill-name>`
- If you introduced repo-level guidance for using the new skill, update `AGENTS.md`

No additional skill registry file needs to be edited.
